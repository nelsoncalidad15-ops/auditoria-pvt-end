...
function formatAuditProgress(audit: IncompleteAuditListItem) {
  ...
  const totalItems = isFromHistory && audit.expectedChildCount
    ? audit.expectedChildCount
    : audit.items?.length || 0;
  const completedItems = isFromHistory && audit.childAudits?.length
    ? audit.childAudits.length
    : audit.items?.filter((item) => item.status).length || 0;
  ...
  return {
    ...
    progressPercent,
    isFromHistory,
    badgeLabel: isFromHistory ? "Proceso" : "Borrador",
    scoreLabel: `${progressPercent}%`,
  };
}
    ...
    <div className="space-y-6 pt-4">
      <div className="flex flex-col gap-3 rounded-[2.2rem] border border-white/80 bg-white/80 p-6 shadow-sm backdrop-blur md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-black tracking-tight text-slate-900">Auditorías pendientes</h2>
          <p className="mt-1 text-sm font-bold text-slate-500">Retomá solamente borradores o procesos incompletos, sin mezclarlo con una auditoría nueva.</p>
        </div>
          ...
          {audits.map((audit) => {
            ...
            return (
              <article
                ...
                onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onResume(audit); } }}
              ...
              >
                ...
                <div className="space-y-3 border-t border-slate-100 pt-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-black text-blue-600">{progress.completedItems}/{progress.totalItems} {progress.isFromHistory ? "areas" : "items"}</span>
                  </div>
